function [like,grad,hess]=clogitHessAdj1(b,restrMat,Y,X,Z,baseAlt,W,adj,beta)
...
if K1==0 && K2>0
	...
	for j=1:J
		% temp= Z(:,:,j)*b2 + beta*adj(:,j);
		temp = Z(:,flagger,j)*b2(flagger) + beta*adj(:,j);
		% assert(isequal(temp1,temp),'temps are off. it must be winter');
		% num = (Y==j).*temp+num;
		num  = (Y==j).*temp+num;
		% assert(isequal(num1,num),'nums are off. it must be accounting');
		% dem = exp(temp)+dem;
		dem  = exp(temp)+dem;
		% assert(isequal(dem1,dem),'dems are off. it must be the RNC');
		% numer(:,j) = exp(temp);
		numer(:,j) = exp(temp);
		% assert(isequal(numer1,numer),'numers are off. it must be accounting');
	end
	P=numer./(dem*ones(1,J));
	% P1=numer1./(dem1*ones(1,J));
	% assert(isequal(P1,P),'Ps are off. it must be the garden');
	...
	% grad1 = zeros(size(b));
	% for j=1:J
		% numg1 = -Z(:,:,j)'*(W.*(Y==j))+numg1;
		% demg1 = -Z(:,:,j)'*(W.*P1(:,j))+demg1;
	% end
	% grad1=numg1-demg1;
	
	grad = zeros(size(b));
	for j=1:J
		numg(flagger) = -Z(:,flagger,j)'*(W.*(Y==j))+numg(flagger);
		demg(flagger) = -Z(:,flagger,j)'*(W.*P(:,j))+demg(flagger);
	end
	...
	for j=1:J
        for jj=1:J
            hess = (Z(:,flagger,j).*(W.*P(:,j)*ones(1,K2)))'*(Z(:,flagger,jj).*(W.*P(:,jj)*ones(1,K2))) + hess;
        end
	end
	grad(flagger)=numg(flagger)-demg(flagger);
	% [grad1 grad]
