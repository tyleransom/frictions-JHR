function [like,grad] = clogitW(b,restrMat,Y,X,Z,baseAlt,W)
...
%   W is a weighting vector, with length N
...
if ~isempty(W)
	assert(size(W,1)==N ,'The length of W should equal the number of observations in Y');
end
...
if nb2>0 && nb>0
    ...
    for j=setdiff(1:nc,baseAlt)
        temp=X*b((k-1)*nb+1:k*nb)+(Z(:,:,j)-Z(:,:,baseAlt))*b2;
        grad((k-1)*nb+1:k*nb)=-X'*(W.*((Y==j)-exp(temp)./dem));
        k = k+1;
    end
    k = 1;
    for j=setdiff(1:nc,baseAlt)
        ...
        demg = -(Z(:,:,j)-Z(:,:,baseAlt))'*(W.*(exp(temp)./dem))+demg;
        k = k+1;
    end
