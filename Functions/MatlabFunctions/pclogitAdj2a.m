function P=pclogitAdj2a(b,b1,Y,X,X1,Z,Z1,nc,baseAlt,adj,beta)
...
if nb2>0 && nb>0
	...
	for j=setdiff(1:nc,baseAlt)
		temp=X*b((k-1)*nb+1:k*nb) + (Z(:,:,j)-Z(:,:,baseAlt))*b2 + X1*b1((k-1)*nb+1:k*nb) + (Z1(:,:,j)-Z1(:,:,baseAlt))*b21 + beta*adj(:,j);
		num(:,j)=exp(temp);
		dem=exp(temp)+dem;
		k = k+1;
	end
elseif nb>0 && nb2==0
	...
	k = 1;
	for j=setdiff(1:nc,baseAlt)
		temp=X*b((k-1)*nb+1:k*nb) + X1*b1((k-1)*nb+1:k*nb);
		num(:,j)=exp(temp);
		dem=exp(temp)+dem;
		k = k+1;
	end
elseif nb==0 && nb2>0
	...
	k = 1;
	for j=setdiff(1:nc,baseAlt)
		temp=(Z(:,:,j)-Z(:,:,baseAlt))*b2 + (Z1(:,:,j)-Z1(:,:,baseAlt))*b21 + beta*(adj(:,j)-adj(:,baseAlt));
		num(:,j)=exp(temp);
		dem=exp(temp)+dem;
		k = k+1;
	end
