...
capture log close
set more off
log using cr_clean_SIPP_2008.log, replace


! gunzip -f [REDACTED]sipp2008NHWmale.csv.gz
insheet using "[REDACTED]sipp2008NHWmale.csv", comma case
! gzip   -f [REDACTED]sipp2008NHWmale.csv
...
*----------------------------
// 752 people have missing population ... 96% of those are because county FIPS is 0 (i.e. I don't know exactly where they live)
...
lab def vlbirthplace 1 "Alabama" 2 "Alaska" 4 "Arizona" 5 "Arkansas" 6 "California" 8 "Colorado" 9 "Connecticut" 10 "Delaware" 11 "DC" 12 "Florida" 13 "Georgia" 15 "Hawaii" 16 "Idaho" 17 "Illinois" 18 "Indiana" 19 "Iowa" 20 "Kansas" 21 "Kentucky" 22 "Louisiana" 23 "Maine" 24 "Maryland" 25 "Massachusetts" 26 "Michigan" 27 "Minnesota" 28 "Mississippi" 29 "Missouri" 30 "Montana" 31 "Nebraska" 32 "Nevada" 33 "New Hampshire" 34 "New Jersey" 35 "New Mexico" 36 "New York" 37 "North Carolina" 38 "North Dakota" 39 "Ohio" 40 "Oklahoma" 41 "Oregon" 42 "Pennsylvania" 44 "Rhode Island" 45 "South Carolina" 46 "South Dakota" 47 "Tennessee" 48 "Texas" 49 "Utah" 50 "Vermont" 51 "Virginia" 53 "Washington" 54 "West Virginia" 55 "Wisconsin" 56 "Wyoming" 555 "Elsewhere" 562 "Northern America" 563 "Northern Europe and Western Europe" 564 "Southern Europe and Eastern Europe" 565 "Eastern Asia" 566 "South Central Asia" 567 "South East Asia, West Asia, Australia, New Zealand" 568 "Africa" 569 "Caribbean" 570 "Central America" 571 "South America" 
...
save [REDACTED]intermediate2008, replace
! gzip   -f [REDACTED]intermediate2008.dta



/*
! gunzip -f [REDACTED]intermediate2008.dta.gz
use [REDACTED]intermediate2008, clear
! gzip   -f [REDACTED]intermediate2008.dta
*/
...
*--------------------------------------------------------------------------------------------
gen yrmo = ym(year,calmonth ) if inlist(timmonth,4,8,12,16,20,24,28,32,36,40,44,48,52,56)
...
****************************************************************************
*********** GET RID OF PEOPLE OUT OF AGE RANGE (should be zero now) ********
****************************************************************************
*------------------------------------------------------------------
...
gen ageFlag = ageFlagA>0 //f[REDACTED]
...
replace WageHr      = WageHr/1.245859466 if year==2009 // adjust for inflation
replace WageHr      = WageHr/1.266295006 if year==2010 // adjust for inflation
replace WageHr      = WageHr/1.306265970 if year==2011 // adjust for inflation
replace WageHr      = WageHr/1.333298490 if year==2012 // adjust for inflation
replace WageHr      = WageHr/1.352828107 if year==2013 // adjust for inflation
...
replace earn        = earn/1.245859466 if year==2009 // adjust for inflation
replace earn        = earn/1.266295006 if year==2010 // adjust for inflation
replace earn        = earn/1.306265970 if year==2011 // adjust for inflation
replace earn        = earn/1.333298490 if year==2012 // adjust for inflation
replace earn        = earn/1.352828107 if year==2013 // adjust for inflation
...
replace derearn0    = total_der_fica_2009+total_der_nonfica_2009 if year==2009
replace derearn0    = total_der_fica_2010+total_der_nonfica_2010 if year==2010
replace derearn0    = total_der_fica_2011+total_der_nonfica_2011 if year==2011
replace derearn0    = totearn_ser_2012                           if year==2012
replace derearn0    = totearn_ser_2013                           if year==2013
...
replace derearn     = (total_der_fica_2009+total_der_nonfica_2009)/1.245859466 if year==2009 // adjust for inflation
replace derearn     = (total_der_fica_2010+total_der_nonfica_2010)/1.266295006 if year==2010 // adjust for inflation
replace derearn     = (total_der_fica_2011+total_der_nonfica_2011)/1.306265970 if year==2011 // adjust for inflation
replace derearn     = (totearn_ser_2012                          )/1.333298490 if year==2012 // adjust for inflation
replace derearn     = (totearn_ser_2013                          )/1.352828107 if year==2013 // adjust for inflation
...
*---------------------------------------------------------------------------------------------------------------
/*
forvalues X=1954/2007 {
gen work2pQ`X' = inrange(wqc_yrtot_`X',3,4)
}
* ignore really old people
egen exper0_alt1 = rowtotal(work2pQ19?? work2pQ2000 work2pQ2001 work2pQ2002 work2pQ2003 work2pQ2004 work2pQ2005 work2pQ2006 work2pQ2007)
egen exper0_altC = rowtotal(wqc_yrtot_pre1953 wqc_yrtot_19?? wqc_yrtot_2000 wqc_yrtot_2001 wqc_yrtot_2002 wqc_yrtot_2003 wqc_yrtot_2004 wqc_yrtot_2005 wqc_yrtot_2006 wqc_yrtot_2007)
...
tab attritFlag if firstper, sum(tenure0)
...
save "[REDACTED]sipp2008NHWmaleTEST.dta", replace
...
preserve
  ...
  compress
  save "[REDACTED]sipp2008NHWmale.dta", replace
  ! gzip -f [REDACTED]sipp2008NHWmale.dta
restore
...
preserve
  ...
  compress
  save `annual', replace
  save "[REDACTED]sipp2008NHWmaleAnnual.dta", replace
  ! gzip -f [REDACTED]sipp2008NHWmaleAnnual.dta
restore

preserve
  tempfile trimesterly
  sum switch_loc
  keep if inlist(timmonth,4,8,12,16,20,24,28,32,36,40,44,48,52,56)
  xtset ID wavemap
  drop switch_state switch_MSA switch_loc switch_county pop_cat_lag
  bys ID (timmonth): generat byte switch_state  =  state[_n-1] ~=state[_n] & _n>1
  bys ID (timmonth): generat byte switch_county =  switch_state | (county[_n-1]~=county[_n] & state[_n-1]==state[_n] & _n>1)
  ...
  save "[REDACTED]sipp2008NHWmaleTrimester.dta", replace
  ! gzip -f [REDACTED]sipp2008NHWmaleTrimester.dta
restore
...
preserve
  ...
  * drop if schoolFlag
  ...
  outsheet using "[REDACTED]sipp2008NHWmale_matlab_wide_annual.csv", comma replace nolabel
  ! gzip -f [REDACTED]sipp2008NHWmale_matlab_wide_annual.csv
restore
