...
*============================================================================
preserve
	insheet using ${pathstem}${source}county_unemp_annual.csv, comma names clear
	drop fips
	* drop if year==2011
...
*****************************************************************************
* read in gross rents from ACS by CBSA code
*****************************************************************************
preserve
	use `counties', clear
	ren cbsa cbsa_code
	* merge
	merge m:1 cbsa_code using ${pathstem}${source}gross_rents, keepusing(grents)
	tab cbsa_code if _merge==1
	tab cbsa_code if _merge==2
	keep if _merge==3
	
	gsort -grents
	l cbsa_code cbsaname grents, sep(0)
...
preserve
	...
	* ren cbsa_code cbsa
	merge m:1 state county using `counties', keepusing (state county cntyname cbsaname)
	drop _merge
	...
	* merge state-level gross rents into county-level file and assign state rents to locations where city rents are unobserved
	...
	l state county grents_urban cbsa_code cntyname cbsaname grents if _merge~=3
	replace grents = grents_urban if ~mi(grents_urban)
	qui sum grents if cntyname=="Atlantic NJ"
	replace grents = `r(mean)' if cntyname=="Cape May NJ"
	l state county grents_urban cbsa_code cntyname cbsaname grents if _merge~=3
	keep  state county cntyname cbsaname cbsa_code grents
...
preserve
	...
	recode cbsa (15680 = 30500) // Lexington Park, MD
	recode cbsa (31080 = 31100) // LA
	recode cbsa (48260 = 44600) // Weirton-Steubenville
	recode cbsa (42200 = 42060) // Santa Barbara
	recode cbsa (33460 = 33460) // Minneapolis
	recode cbsa (14010 = 14060) // Bloomington, IL
	recode cbsa (46520 = 26180) // Honolulu, HI
	recode cbsa (29200 = 29140) // Lafayette, IN
	* tab cbsa
	ren cbsa cbsa_code
	sort cbsa_code
...
preserve
	insheet using ${pathstem}${source}msagdpPerCapita2012.csv, comma names clear
	recode cbsa (15680 = 30500) // Lexington Park, MD
	recode cbsa (31080 = 31100) // LA
	recode cbsa (48260 = 44600) // Weirton-Steubenville
	recode cbsa (42200 = 42060) // Santa Barbara
...
* get CBSA codes for problem CBSAs:
* tab cbsa_code if regexm(cbsaname,"Bloomington") | regexm(cbsaname,"Honolulu") | regexm(cbsaname,"California") | regexm(cbsaname,"Lafayette") | regexm(cbsaname,"Barbara") | regexm(cbsaname,"Weirton")
* tab cbsaname if regexm(cbsaname,"Bloomington") | regexm(cbsaname,"Honolulu") | regexm(cbsaname,"California") | regexm(cbsaname,"Lafayette") | regexm(cbsaname,"Barbara") | regexm(cbsaname,"Weirton")
merge 1:1 cbsa_code using `msa_gdp'
tab cbsaname if _merge==2
...
gen tester2013 = 1
gen tester2012 = 2
reshape long gdpPerCapita gdp accra grocery housing utilities transport health misc weight_composite_index weight_grocery_items weight_housing weight_utilities weight_transportation weight_health_care weight_misc_goods_services tester, i(state county) j(year)
drop tester
...
foreach var in accra grocery housing grents utilities transport health misc sgrt shot sutt shet strt smit {
	bys state county (year): replace `var' = `var'[19] if year>=2009 // let ACCRA in 2009 and 2010 be the same as 2008
}
sum sgrt shot sutt shet strt smit
gen sum_shares=sgrt+shot+sutt+shet+strt+smit
assert inrange(sum_shares,.99,1.01) if ~mi(sum_shares)
*****************************************************************************
...
keep state county year cbsa_code cbsaname stabb cntyname pop?? landarea dens?? latitude longitude gdp* rural csize region tempcpi CPI_Ta CPI_Tb grents housing labor_force employed unemployed urate erate lfrate00 ua_pct10 area_ua area_cou `BEAvars'
...
gen lnpop = ln(pop00)
gen lngdp = ln(gdpPerCapita000)
graph twoway (scatter lngdp lnpop if year==2004, mlabel(stabb))
graph export gdp_pop_outliers.eps, replace 

* Scatter plot of log income and population to see relationship I'm interested in describing
gen lnipc  = ln(incPerCapita/(tempcpi/100))
gen lnipcA = ln(incPerCapita/CPI_Ta)
gen lnipcB = ln(incPerCapita/CPI_Tb)
* Year 1990
reg lnipc lnpop if year==1990 & mainObs & cbsa_code~=99999
predict hat
graph twoway (scatter lnipc lnpop if year==1990 & mainObs & cbsa_code~=99999) (line hat lnpop if year==1990 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot1990.eps, replace

reg lnipcA lnpop if year==1990 & mainObs & cbsa_code~=99999
predict hatA
graph twoway (scatter lnipcA lnpop if year==1990 & mainObs & cbsa_code~=99999) (line hatA lnpop if year==1990 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Ta1990.eps, replace

reg lnipcB lnpop if year==1990 & mainObs & cbsa_code~=99999
predict hatB
graph twoway (scatter lnipcB lnpop if year==1990 & mainObs & cbsa_code~=99999) (line hatB lnpop if year==1990 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Tb1990.eps, replace

* Year 2000
drop hat hatA hatB
reg lnipc lnpop if year==2000 & mainObs & cbsa_code~=99999
predict hat
graph twoway (scatter lnipc lnpop if year==2000 & mainObs & cbsa_code~=99999) (line hat lnpop if year==2000 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot2000.eps, replace

reg lnipcA lnpop if year==2000 & mainObs & cbsa_code~=99999
predict hatA
graph twoway (scatter lnipcA lnpop if year==2000 & mainObs & cbsa_code~=99999) (line hatA lnpop if year==2000 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Ta2000.eps, replace

reg lnipcB lnpop if year==2000 & mainObs & cbsa_code~=99999
predict hatB
graph twoway (scatter lnipcB lnpop if year==2000 & mainObs & cbsa_code~=99999) (line hatB lnpop if year==2000 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Tb2000.eps, replace

* Year 2006 (pre-recession boom)
drop hat hatA hatB
reg lnipc lnpop if year==2006 & mainObs & cbsa_code~=99999
predict hat
graph twoway (scatter lnipc lnpop if year==2006 & mainObs & cbsa_code~=99999) (line hat lnpop if year==2006 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot2006.eps, replace

reg lnipcA lnpop if year==2006 & mainObs & cbsa_code~=99999
predict hatA
graph twoway (scatter lnipcA lnpop if year==2006 & mainObs & cbsa_code~=99999) (line hatA lnpop if year==2006 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Ta2006.eps, replace

reg lnipcB lnpop if year==2006 & mainObs & cbsa_code~=99999
predict hatB
graph twoway (scatter lnipcB lnpop if year==2006 & mainObs & cbsa_code~=99999) (line hatB lnpop if year==2006 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Tb2006.eps, replace

* Year 2009 (post-recession bust)
drop hat hatA hatB
reg lnipc lnpop if year==2009 & mainObs & cbsa_code~=99999
predict hat
graph twoway (scatter lnipc lnpop if year==2009 & mainObs & cbsa_code~=99999) (line hat lnpop if year==2009 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot2009.eps, replace

reg lnipcA lnpop if year==2009 & mainObs & cbsa_code~=99999
predict hatA
graph twoway (scatter lnipcA lnpop if year==2009 & mainObs & cbsa_code~=99999) (line hatA lnpop if year==2009 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Ta2009.eps, replace

reg lnipcB lnpop if year==2009 & mainObs & cbsa_code~=99999
predict hatB
graph twoway (scatter lnipcB lnpop if year==2009 & mainObs & cbsa_code~=99999) (line hatB lnpop if year==2009 & mainObs & cbsa_code~=99999, sort), xtitle("log population") ytitle("log income per capita") xlabel(9(1)17) ylabel(2(.5)5) legend(label(1 "log income per capita") label(2 "fitted values") cols(1)) note("Notes: N = `e(N)'" "beta = `=round(_b[lnpop],.0001)'" "R-squared = `=round(`e(r2)',.0001)'")
graph export inc_pop_plot_CPI_Tb2009.eps, replace

* Scatter plot of unemployment rate and time to see relationship I'm interested in describing
preserve
collapse (first) urate pop00 pop10 cbsaname, by(cbsa_code year)
bys year (cbsa_code): egen rank = rank(pop10), field
sort cbsa_code year
keep if rank<=30
lab var urate "Unemployment Rate"
lab var year "Year"
lab def vlyear 1990 "1990" 1994 "1994" 1998 "1998" 2002 "2002" 2006 "2006" 2010 "2010"
lab val year vlyear
codebook year
graph twoway (scatter urate year if inrange(year,1990,2012)), xlabel(1990(4)2010) ylabel(0(0.01)0.15)
graph export urate_over_time.eps, replace
 * xlabel(1990(4)2010)
graph box urate if inrange(year,1990,2012), over(year, relabel(1 "1990" 2 " " 3 " " 4 " " 5 "1994" 6 " " 7 " " 8 " " 9 "1998" 10 " " 11 " " 12 " " 13 "2002" 14 " " 15 " " 16 " " 17 "2006" 18 " " 19 " " 20 " " 21 "2010" 22 " " 23 " ")) b1title("Year") ylabel(0(0.01)0.15)
graph export urate_over_time_box.eps, replace
restore
...
xtile col_pctile = CPI_Tb      if year==2004, nq(100)
xtile hpi_pctile = grents      if year==2004, nq(100)
...
egen  col_rank = rank(CPI_Tb      ) if year==2004 & mainObs, field
egen  hpi_rank = rank(grents      ) if year==2004 & mainObs, field
l cbsaname pop00 pop_pctile pop_rank gdp gdp_pctile gdp_rank incPerCapita ipc_pctile ipc_rank CPI_Ta CPI_Tb grents housing if year==2004 & mainObs & ( regexm(cbsaname,"Rivers") | regexm(cbsaname, "Casp") | regexm(cbsaname, "New Y") | regexm(cbsaname ,"The Villages") | regexm(cbsaname ,"Sebring"))

l cbsaname pop00 pop_pctile pop_rank gdpPerCapita gdp_pctile gdp_rank incPerCapita ipc_pctile ipc_rank CPI_Tb col_pctile col_rank grents hpi_pctile hpi_rank CPI_Ta housing if year==2004 & mainObs & ( regexm(cbsaname,"Rivers") | regexm(cbsaname, "Casp") | regexm(cbsaname, "New Y") | regexm(cbsaname ,"The Villages") | regexm(cbsaname ,"Sebring"))

l cbsaname pop00 pop_pctile pop_rank gdpPerCapita gdp_pctile gdp_rank incPerCapita ipc_pctile ipc_rank CPI_Tb col_pctile col_rank grents hpi_pctile hpi_rank CPI_Ta housing if year==2004 & mainObs & ( regexm(cbsaname,"Provo") | regexm(cbsaname, "Durh"))
