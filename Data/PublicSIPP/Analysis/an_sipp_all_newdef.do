...
set more off
capture log close
log using an_sipp_all_newdef.log, replace
...
gen inlf    = (inrange(rhrsweek,1,6) | (rhrsweek==0 & look_work==1)) // spent at least one week working positive hours; or spent any amount of time looking for work
gen empFT   =  inrange(rhrsweek,1,6) & inlf
gen emp     =  inrange(rhrsweek,1,6) & inlf
...
*----------------------------------------------------
gen racer = .
replace racer = 1 if race==1 & eorigin==2
replace racer = 2 if race==2 & eorigin==2
replace racer = 3 if eorigin==1
replace racer = 4 if mi(racer)
lab def vlracer 1 "White" 2 "Black" 3 "Hispanic" 4 "Other"
lab val racer vlracer

reg empFT i.racer##i.sex c.exper##c.exper        mover_dummy if empFT_prev==1
reg empFT i.racer##i.sex c.exper##c.exper        mover_dummy if empFT_prev==0
reg empFT i.racer##i.sex c.exper##c.exper lurate mover_dummy if empFT_prev==1
qui predict pEmpFT1 if e(sample), xb 
reg empFT i.racer##i.sex c.exper##c.exper lurate if empFT_prev==1
qui predict pEmpFT1r if e(sample), resid 
reg empFT i.racer##i.sex c.exper##c.exper lurate mover_dummy if empFT_prev==0
qui predict pEmpFT0 if e(sample), xb 
reg empFT i.racer##i.sex c.exper##c.exper lurate if empFT_prev==0
qui predict pEmpFT0r if e(sample), resid 
qui generat pEmpFT = .
qui replace pEmpFT = pEmpFT1 if empFT_prev==1
qui replace pEmpFT = pEmpFT0 if empFT_prev==0
qui generat pEmpFTm0 = .
qui generat pEmpFTm1 = .
qui replace pEmpFTm1 = pEmpFT if mover_dummy==1
qui replace pEmpFTm0 = pEmpFT if mover_dummy==0

lab def vlempFTp 0 "Non-employed" 1 "Employed"
lab val empFT_prev vlempFTp

lab def vlmover 0 "Stayers" 1 "Movers"
lab val mover_dummy vlmover

generat epstatus = .
replace epstatus = 1 if empFT_prev

set scheme s2color

graph bar pEmpFTm0 pEmpFTm1, over(mover_dummy, gap(-200) relabel(1 "Stayers                              " 2 "                        Movers")) over(empFT_prev, gap(30)) ytitle("Pr(Employed)") graphregion(color(oucream)) plotregion(color(oucream)) legend(off) bar(2, fintensity(inten40))
graph export barPempNewDef.eps, replace

logit empFT i.racer##i.sex c.exper##c.exper lurate mover_dummy if empFT_prev==1
qui predict pEmpFTlogit1 if e(sample), pr 
logit empFT i.racer##i.sex c.exper##c.exper lurate mover_dummy if empFT_prev==0
qui predict pEmpFTlogit0 if e(sample), pr 
qui generat pEmpFTlogit = .
qui replace pEmpFTlogit = pEmpFTlogit1 if empFT_prev==1
qui replace pEmpFTlogit = pEmpFTlogit0 if empFT_prev==0
qui generat pEmpFTmlogit0 = .
qui generat pEmpFTmlogit1 = .
qui replace pEmpFTmlogit1 = pEmpFTlogit if mover_dummy==1
qui replace pEmpFTmlogit0 = pEmpFTlogit if mover_dummy==0

graph bar pEmpFTmlogit0 pEmpFTmlogit1, over(mover_dummy, gap(-200) relabel(1 "Stayers                              " 2 "                        Movers")) over(empFT_prev, gap(30)) ytitle("Pr(Employed)") graphregion(color(oucream)) plotregion(color(oucream)) legend(off) bar(2, fintensity(inten40))
graph export barPempLogitNewDef.eps, replace

/*
preserve
    collapse pEmpFT status, by(empFT_prev mover_dummy)
    graph twoway (bar pEmpFTm0 pEmpFTm1, over(empFT_prev))  ///
                 (scatter pEmpFTm0 pEmpFTm1, m(i) mlabel(mover_dummy) mlabposition(12)), ///
                  legend(off) ytitle("Pr(Employed)") bgcol(oucream) graphregion(color(oucream)) plotregion(color(oucream))
    graph export barPemp.eps, replace
restore
*/
...
*----------------------------------------------------
* Graphs -- classic scheme
*----------------------------------------------------
set scheme s2color

generat MRhouse = mover==2
generat MRcity  = mover==3
generat MRstate = mover==4
replace MRhouse = 3*(mover==2) if wave==2
...
preserve
    ...
    l, sepby(year)
    generat MRSameCounty   = 100*MRhouse
    generat MRSameState    = 100*MRcity
    generat MRDiffState    = 100*MRstate   

    graph set eps fontface "Palatino Linotype"
    graph twoway (line MRSameCounty   year if inrange(year,2004,2013) & empFT_prev==1, lpattern(.) lcolor(black) )  /// between counties
                 (line MRSameState    year if inrange(year,2004,2013) & empFT_prev==1, lpattern(-) lcolor(black) )  /// between states
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT_prev==1, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(1 "Within county") label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(1)`=4*`cheight'') xlabel(2004(1)2013) graphregion(color(white))
    graph export mobility_all_employed_moredist_newdef.eps, replace

    graph twoway (line MRSameCounty   year if inrange(year,2004,2013) & empFT_prev==0, lpattern(.) lcolor(black) )  /// between counties
                 (line MRSameState    year if inrange(year,2004,2013) & empFT_prev==0, lpattern(-) lcolor(black) )  /// between states
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT_prev==0, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(1 "Within county") label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(1)`=4*`cheight'') xlabel(2004(1)2013) graphregion(color(white))
    graph export mobility_all_nonemployed_moredist_newdef.eps, replace

    graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT_prev==1, lpattern(.) lcolor(black) )  /// between counties
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT_prev==1, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(1 "Between counties, within state") label(2 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
    graph export mobility_all_employed_newdef.eps, replace

    graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT_prev==0, lpattern(.) lcolor(black) )  /// between counties
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT_prev==0, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(1 "Between counties, within state") label(2 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
    graph export mobility_all_nonemployed_newdef.eps, replace
restore
...
preserve
    ...
    graph set eps fontface "Palatino Linotype"
    graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT==1, lpattern(.) lcolor(black) )  /// between counties
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT==1, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
    graph export mobility_all_curr_employed_newdef.eps, replace

    graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT==0, lpattern(.) lcolor(black) )  /// between counties
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT==0, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
    graph export mobility_all_curr_nonemployed_newdef.eps, replace
restore
*/


*----------------------------------------------------
* Graphs -- uncluttered publication scheme
*----------------------------------------------------
set scheme uncluttered_cream
...
preserve
    ...
    gen str15 dist1_label =""
    gen str15 dist2_label =""
    gen str15 dist3_label =""
    qui sum year
    local maxyear = `r(max)'
    qui sum MRSameCounty if year==`maxyear'
    local mcounty = `r(mean)'
    qui sum MRSameState if year==`maxyear'
    local mstate = `r(mean)'
    qui sum MRDiffState if year==`maxyear'
    local mreg = `r(mean)'
    replace dist1_label = "Within County"  if year==`maxyear' & inlist(empFT_prev,0,1)
    replace dist2_label = "Within State"   if year==`maxyear' & inlist(empFT_prev,0,1)
    replace dist3_label = "Between States" if year==`maxyear' & inlist(empFT_prev,0,1)
    local label1_opts "mlab(dist1_label) msize(zero) mlabsize(large)"
    local label2_opts "mlab(dist2_label) msize(zero) mlabsize(large)"
    local label3_opts "mlab(dist3_label) msize(zero) mlabsize(large)"

    graph set eps fontface "Palatino Linotype"
    graph twoway (connected MRSameCounty year if inrange(year,2004,2013) & empFT_prev==1, `label1_opts')  ///
                 (connected MRSameState  year if inrange(year,2004,2013) & empFT_prev==1, `label2_opts')  ///
                 (connected MRDiffState  year if inrange(year,2004,2013) & empFT_prev==1, `label3_opts'), ///
    ytitle("") yscale(range(0(2)12)) ylabel(0(2)12) xtitle("") xscale(range(2004(1)2016)) xlabel(2004(1)2013) graphregion(color(oucream))
    graph export mobility_all_employed_moredistcream_newdef.eps, replace

    graph twoway (connected MRSameCounty year if inrange(year,2004,2013) & empFT_prev==0, `label1_opts')  ///
                 (connected MRSameState  year if inrange(year,2004,2013) & empFT_prev==0, `label2_opts')  ///
                 (connected MRDiffState  year if inrange(year,2004,2013) & empFT_prev==0, `label3_opts'), ///
    ytitle("") yscale(range(0(2)12)) ylabel(0(2)12) xtitle("") xscale(range(2004(1)2016)) xlabel(2004(1)2013) graphregion(color(oucream))
    graph export mobility_all_nonemployed_moredistcream_newdef.eps, replace

    graph twoway (connected MRSameState    year if inrange(year,2004,2013) & empFT_prev==1, `label2_opts')  ///
                 (connected MRDiffState    year if inrange(year,2004,2013) & empFT_prev==1, `label3_opts'), ///
    ytitle("") yscale(range(0(.5)3.5)) ylabel(0(.5)3.5) xtitle("") xscale(range(2004(1)2016)) xlabel(2004(1)2013) graphregion(color(oucream))
    graph export mobility_all_employedcream_newdef.eps, replace

    graph twoway (connected MRSameState    year if inrange(year,2004,2013) & empFT_prev==0, `label2_opts')  ///
                 (connected MRDiffState    year if inrange(year,2004,2013) & empFT_prev==0, `label3_opts'), ///
    ytitle("") yscale(range(0(.5)3.5)) ylabel(0(.5)3.5) xtitle("") xscale(range(2004(1)2016)) xlabel(2004(1)2013) graphregion(color(oucream))
    graph export mobility_all_nonemployedcream_newdef.eps, replace
restore
...
preserve
    ...
    graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT==1, lpattern(.) lcolor(black) )  /// between counties
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT==1, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(oucream))
    graph export mobility_all_curr_employedcream_newdef.eps, replace

    graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT==0, lpattern(.) lcolor(black) )  /// between counties
                 (line MRDiffState    year if inrange(year,2004,2013) & empFT==0, lpattern(-) lcolor(black) ), /// between states
              ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
                 legend(label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(oucream))
    graph export mobility_all_curr_nonemployedcream_newdef.eps, replace
restore
*/
