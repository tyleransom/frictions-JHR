...
set more off
capture log close
log using an_sipp_men.log, replace
...
forvalues k=1(1)12 {
    ...
    keep if srefmon==4 & eeducate<44 & esex==1
    tab1 swave srefmon
}
...
reg empFT i.racer c.exper##c.exper        mover_dummy if empFT_prev==1
reg empFT i.racer c.exper##c.exper        mover_dummy if empFT_prev==0
reg empFT i.racer c.exper##c.exper lurate mover_dummy if empFT_prev==1
reg empFT i.racer c.exper##c.exper lurate mover_dummy if empFT_prev==0
...
preserve
	...
	graph export mobility_men_employed.eps, replace
	...
	graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT_prev==0, lpattern(.) lcolor(black) )  /// between counties
	          ...
	          ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
				 legend(label(1 "Between counties, within state") label(2 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
	graph export mobility_men_nonemployed.eps, replace
restore
...
preserve
	...
	graph export mobility_men_curr_employed.eps, replace
	...
	graph twoway (line MRSameState    year if inrange(year,2004,2013) & empFT==0, lpattern(.) lcolor(black) )  /// between counties
	          ...
	          ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
				 legend(label(2 "Between counties, within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
	graph export mobility_men_curr_nonemployed.eps, replace
restore
