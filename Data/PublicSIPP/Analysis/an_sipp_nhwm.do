...
set more off
capture log close
log using an_sipp_nhwm.log, replace
...
forvalues k=1(1)12 {
    ...
    keep if srefmon==4 & esex==1 & erace==1 & eeducate<44 & !inrange(eorigin,1,1)
    tab1 swave srefmon
}
...
bys ID (timmonth): gen jb1ID_prev = job1id[_n-1]
bys ID (timmonth): gen jb2ID_prev = job2id[_n-1]
...
gen job1switch = job1id!=jb1ID_prev & !mi(job1id) & !mi(jb1ID_prev)
gen job1SEL    = job1id!=jb1ID_prev
gen job2switch = job2id!=jb2ID_prev & !mi(job2id) & !mi(jb2ID_prev)
...
preserve
	...
	graph export mobility_nhwm_employed.eps, replace
	...
	graph twoway ///(area c2             year if inrange(year,2004,2013)                ,              color(gs15)  )  /// recession
				 ...
				 legend(label(1 "Recession") label(2 "Within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
	graph export mobility_nhwm_nonemployed.eps, replace
restore
...
preserve
	...
	graph twoway (area c2             year if inrange(year,2004,2013)           ,              color(gs15)  )  /// recession
				 ...
				 legend(label(1 "Recession") label(2 "Within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
	graph export mobility_nhwm_curr_employed.eps, replace
	...
	graph twoway (area c2             year if inrange(year,2004,2013)           ,              color(gs15)  )  /// recession
				 ...
				 legend(label(1 "Recession") label(2 "Within state") label(3 "Between states") cols(2)) ytitle("Annual Migration Rate (%)") xtitle("Year") ylabel(0(.5)`cheight') xlabel(2004(1)2013) graphregion(color(white))
	graph export mobility_nhwm_curr_nonemployed.eps, replace
restore
