...
pimat1w(1,1,:) = lambdervecRhoCflPiIntS(nloc,l,Tmater,yr-2003,urate_lag55,0,1,experer,typer,rho_hat_urate,1,squeeze(shocker(1,(l-1)*2+2,:))',bLemp,bLunemp,l,shock_amt);
for jj=1:nloc 
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc
            ...
            if j==lp
            pim = 1-lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,typer,rho_hat_urate,1,shockd1,bLemp,bLunemp,l,shock_amt);
            % size(pim)
            ...
            pim = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,typer,rho_hat_urate,1,shockd1,bLemp,bLunemp,l,shock_amt);
            Ztemp(1,:,j,:) = [pim zeros(1,1,draws) ewagevecRhoIntS(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,typer,bwage,shockw1)  repmat(birthLoc(1,j),[1 1 draws]) repmat(birthDiv(1,j),[1 1 draws]) repmat(switchervecS(nloc,k,j,ager+1,typer),[1 1 draws]) repmat(movervecS(nloc,k,j,distance,ager+1,1,0,typer),[1 1 draws])];
            end
        else
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            ...
            if j==lp
            pim = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,typer,rho_hat_urate,1,shockl1,bLemp,bLunemp,l,shock_amt);
            ...
            pim = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,typer,rho_hat_urate,1,shockl1,bLemp,bLunemp,l,shock_amt);
            Ztemp(1,:,j,:) = [zeros(1,1,draws) pim ewagevecRhoIntS(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,typer,bwage,shockw1) repmat(birthLoc(1,j),[1 1 draws]) repmat(birthDiv(1,j),[1 1 draws]) repmat(switchervecS(nloc,k,j,ager+1,typer),[1 1 draws]) repmat(movervecS(nloc,k,j,distance,ager+1,0,1,typer),[1 1 draws])];
            end
        else
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            shockw1 = squeeze(shocker(1,(j-1)*2+1,:))';
            shockl1 = squeeze(shocker(1,(j-1)*2+2,:))';
            entry1 = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer+jp,typer,rho_hat_urate,2,shockl1,bLemp,bLunemp,l,shock_amt);
    		entry2 = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer+jp,typer,rho_hat_urate,2,shockl1,bLemp,bLunemp,l,shock_amt);
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            shockw1 = squeeze(shocker(1,(j-1)*2+1,:))';
            shockl1 = squeeze(shocker(1,(j-1)*2+2,:))';
            entry1 = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,typer,rho_hat_urate,2,shockl1,bLemp,bLunemp,l,shock_amt);
    		entry2 = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,typer,rho_hat_urate,2,shockl1,bLemp,bLunemp,l,shock_amt);
			pim = entry1.*(homeLocW==j)+entry2.*(homeLocW~=j);
...
for jj=1:nloc
	pimat1w(1,1,:) = lambdervecRhoCflPiIntS(nloc,l,Tmater,yr-2003,urate_lag55,0,1,experer,typer,rho_hat_urate,1,squeeze(shocker(1,(l-1)*2+2,:))',bLemp,bLunemp,l,shock_amt); % f(rho,lambda_hat(l)); %not sure how log-odds AR(1) interfaces here
    omegaw(1,jj,:) = repmat(pimat(1,jj),[1 1 draws])./pimat1w(1,1,:);
end
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            shockw1 = squeeze(shocker(1,(j-1)*2+1,:))';
            shockl1 = squeeze(shocker(1,(j-1)*2+2,:))';
        	entry1 = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,typer,rho_hat_urate,1,shockl1,bLemp,bLunemp,l,shock_amt);
        	entry2 = lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,typer,rho_hat_urate,1,shockl1,bLemp,bLunemp,l,shock_amt);
        	pim = entry1.*(homeLocW==j)+entry2.*(homeLocW~=j);
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc
            shockw1 = squeeze(shocker(1,(j-1)*2+1,:))';
            shockd1 = squeeze(shocker(1,(j-1)*2+2,:))';
        	entry1 = 1-lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,typer,rho_hat_urate,2,shockd1,bLemp,bLunemp,l,shock_amt);
        	entry2 =   lambdervecRhoCflPiIntS(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,typer,rho_hat_urate,2,shockd1,bLemp,bLunemp,l,shock_amt);
        	pim = entry1.*(homeLocW==j)+entry2.*(homeLocW~=j);
