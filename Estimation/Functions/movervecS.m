function MC = movervecS(nloc,k,j,dist,a,prevEmp,prevUnemp,type)
...
kp = k-nloc*(k>nloc);
jp = j-nloc*(j>nloc);
MC = [(kp~=jp) disttricky(1000*kp+jp,1) disttricky(1000*kp+jp,1).^2 a.*(kp~=jp) (a.^2).*(kp~=jp) prevEmp.*(kp~=jp) prevUnemp.*(kp~=jp) (type==1).*(kp~=jp)];
