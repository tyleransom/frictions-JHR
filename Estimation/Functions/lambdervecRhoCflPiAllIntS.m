function lambda = lambdervecRhoCflPiAllInt(nloc,j,Tmat,Tvec,u,empStat,here,e,type,r,t,err,be,bu,corrmat,shock_amt)
...
for jj=1:nloc
	...
	for tt=1:size(Tmat,2)
        ...
        if isscalar(flag)
            if flag
                Umat(1,1,:) = repmat((r1(flag).*(t==1)+r1(flag).*(1+r2(flag)).*(t==2))+(r2(flag).^t).*(u(jj,tt)+shockley(jj)),[1 1 D])+reshape(err(flag,:),[sum(flag) 1 D]);
            end
        else
            Umat(flag,1,:) = repmat((r1(flag).*(t==1)+r1(flag).*(1+r2(flag)).*(t==2))+(r2(flag).^t).*(u(jj,tt)+shockley(jj)),[1 1 D])+reshape(err(flag,:),[sum(flag) 1 D]);
        end
	end
end
if all(j>nloc)
