function lambda = lambdervecRhoCflPiAllS(nloc,j,Tmat,Tvec,u,empStat,here,e,type,r,t,be,bu,corrmat,shock_amt)
...
for jj=1:nloc
	...
	for tt=1:size(Tmat,2)
		flag = j==jj & Tmat(:,tt)==1;
		Umat(flag) = (r1(flag).*(t==1)+r1(flag).*(1+r2(flag)).*(t==2))+(r2(flag).^t).*(u(jj,tt)+shockley(jj));
	end
end
if all(j>nloc)
	lambda = zeros(size(e));
	...
	if empStat==1
		if here==1
			X = [ones(size(e)) locer 100*Umat zeros(size(e)) e e.^2./100 type==1];
			p = 1./(1+exp(X*be));
		else
			X = [ones(size(e)) locer 100*Umat ones(size(e)) e e.^2./100 type==1];
