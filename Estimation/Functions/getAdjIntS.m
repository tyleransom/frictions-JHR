function [Adj] = getAdjIntS(b,draws,ARcov,ARcov2,yr,ager,LEi,LYi,experer,typer,birthLoc,birthDiv,urate_lag55,J,rho_hat_urate,bLemp,bLunemp,rho_hat_wage,bwage,distance,baseAlt,Beta)
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc
            ...
            %size(pim)
            %size(ewagevecRhoIntS(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,typer,bwage,shockw1))
            %size(repmat(birthLoc(1,j),[1 1 draws]))
            %size(repmat(birthDiv(1,j),[1 1 draws]))
            %size(repmat(switchervecS(nloc,k,j,ager+1,typer),[1 1 draws]))
            %size(repmat(movervecS(nloc,k,j,distance,ager+1,1,0,typer),[1 1 draws]))
