function [Adj] = getAdj(b,yr,ager,LEi,LYi,experer,birthLoc,birthDiv,urate_lag55,J,rho_hat_urate,bLemp,bLunemp,rho_hat_wage,bwage,distance,baseAlt,Beta)
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc
            if j==lp
            Ztemp(1,:,j) = [1-lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,rho_hat_urate,1,bLemp,bLunemp) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) (lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,rho_hat_urate,1,bLemp,bLunemp)).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            else
            Ztemp(1,:,j) = [lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,rho_hat_urate,1,bLemp,bLunemp) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,rho_hat_urate,1,bLemp,bLunemp).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            end
        else
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            if j==lp
            Ztemp(1,:,j) = [lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,1,bLemp,bLunemp) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,1,bLemp,bLunemp).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            else
            Ztemp(1,:,j) = [lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,rho_hat_urate,1,bLemp,bLunemp) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) lambdervecRho(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,rho_hat_urate,1,bLemp,bLunemp).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            end
        else
            Ztemp(1,:,j) = [zeros(1,3) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
        end
