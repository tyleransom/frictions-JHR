function [Adj] = getAdjCflPi(b,yr,ager,LEi,LYi,experer,birthLoc,birthDiv,urate_lag55,J,rho_hat_urate,bLemp,bLunemp,rho_hat_wage,bwage,shock_amt,distance,baseAlt,Beta)
...
for k=1:nloc
    if LEi==0 && (k==LYi-nloc*(LYi>nloc)); 
       pimat(1,k)  = lambdervecRhoCflPi(nloc,k,Tmater,yr-2003,urate_lag55,LEi==0,1,experer,rho_hat_urate  ,0,bLemp,bLunemp,l,shock_amt);
    ...
    elseif LEi==0 && (k~=LYi-nloc*(LYi>nloc));
       pimat(1,k)  = lambdervecRhoCflPi(nloc,k,Tmater,yr-2003,urate_lag55,LEi==0,0,experer,rho_hat_urate,0,bLemp,bLunemp,l,shock_amt);
    elseif LEi==1 && (k~=LYi-nloc*(LYi>nloc)); 
    ...
    if (k==LYi-nloc*(LYi>nloc)); 
       pimat1(1,1) = lambdervecRhoCflPi(nloc,k,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt);
    end
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc
            if j==lp
            Ztemp(1,:,j) = [1-lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) (lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt)).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            else
            Ztemp(1,:,j) = [lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            end
        else
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            if j==lp
            Ztemp(1,:,j) = [lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            else
            Ztemp(1,:,j) = [lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt) ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt).*ewagevecRho(nloc,j,Tmater,yr-2003,rho_hat_wage,1,experer,bwage) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
            end
        else
            Ztemp(1,:,j) = [zeros(1,3) birthLoc(1,j) birthDiv(1,j) switchervec(nloc,k,j,ager+1) movervec(nloc,k,j,distance,ager+1)];
        end
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc 
            entry1 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer+jp,rho_hat_urate,2,bLemp,bLunemp,l,shock_amt);
            entry2 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer+jp,rho_hat_urate,2,bLemp,bLunemp,l,shock_amt);
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            entry1 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,2,bLemp,bLunemp,l,shock_amt);
            entry2 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,rho_hat_urate,2,bLemp,bLunemp,l,shock_amt);
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            entry1 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,1,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt);
            entry2 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,0,0,experer,rho_hat_urate,1,bLemp,bLunemp,l,shock_amt);
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            entry1 = 1-lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,1,1,experer+jp,rho_hat_urate,2,bLemp,bLunemp,l,shock_amt);
            entry2 = lambdervecRhoCflPi(nloc,j,Tmater,yr-2003,urate_lag55,1,0,experer+jp,rho_hat_urate,2,bLemp,bLunemp,l,shock_amt);
