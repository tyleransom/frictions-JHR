function [Adj] = getAdjCflWageIntS(b,draws,ARcov,ARcov2,yr,ager,LEi,LYi,experer,typer,birthLoc,birthDiv,urate_lag55,J,rho_hat_urate,bLemp,bLunemp,rho_hat_wage,bwage,shock_amt,distance,baseAlt,Beta)
...
for k=1:J
    ...
    for j=1:J
        if j<=nloc
            ...
            % size(pim)
            % size(ewagevecRhoCflWageIntS(nloc,l,shock_amt,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,typer,bwage,shockw1))
            % size(repmat(movervecS(nloc,k,j,distance,ager+1,1,0),[1 1 draws]))
            Ztemp(1,:,j,:) = [pim zeros(1,1,draws) ewagevecRhoCflWageIntS(nloc,l,shock_amt,j,Tmater,yr-2003,rho_hat_wage,1,experer+jp,typer,bwage,shockw1)  repmat(birthLoc(1,j),[1 1 draws]) repmat(birthDiv(1,j),[1 1 draws]) repmat(switchervecS(nloc,k,j,ager+1,typer),[1 1 draws]) repmat(movervecS(nloc,k,j,distance,ager+1,1,0,typer),[1 1 draws])];
            else
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
            ...
            Ztemp(1,:,j,:) = [zeros(1,1,draws) pim ewagevecRhoCflWageIntS(nloc,l,shock_amt,j,Tmater,yr-2003,rho_hat_wage,1,experer,typer,bwage,shockw1) repmat(birthLoc(1,j),[1 1 draws]) repmat(birthDiv(1,j),[1 1 draws]) repmat(switchervecS(nloc,k,j,ager+1,typer),[1 1 draws]) repmat(movervecS(nloc,k,j,distance,ager+1,0,1,typer),[1 1 draws])];
            else
...
for k=1:J
    ...
    for j=1:J 
        if j<=nloc
			...
			pim = entry1.*(homeLocW==j)+entry2.*(homeLocW~=j);
            Ztemp(1,:,j,:) = [zeros(1,1,draws) pim ewagevecRhoCflWageIntS(nloc,l,shock_amt,j,Tmater,yr-2003,rho_hat_wage,2,experer,typer,bwage,shockw1) repmat(birthLoc(1,j),[1 1 draws]) repmat(birthDiv(1,j),[1 1 draws]) repmat(switchervecS(nloc,nloc+lp,j,ager+2,typer),[1 1 draws]) repmat(movervecS(nloc,nloc+lp,j,distance,ager+2,0,0,typer),[1 1 draws])];
        else
            Ztemp(1,:,j,:) = [zeros(1,3,draws) repmat(birthLoc(1,j),[1 1 draws]) repmat(birthDiv(1,j),[1 1 draws]) repmat(switchervecS(nloc,nloc+lp,j,ager+2,typer),[1 1 draws]) repmat(movervecS(nloc,nloc+lp,j,distance,ager+2,0,0,typer),[1 1 draws])];
        end
    end
